...
function LineChart({ ChartData, dataSelection }) {
  ...
  useEffect(() => {
    ...
    ChartData.map((displayGroupItems, i) => {
      displayGroupItems.color = colors(i);
      Ydomain.push(displayGroupItems.value);
      Xdomain.push(parse(displayGroupItems.date));
      console.log(displayGroupItems.date);
    });

    console.log(d3.extent(Xdomain));
    ...
    valuePaths.data(ChartData).join(
      (enter) =>
        enter
          .append("path")
    ...
    valuePoints.data(ChartData).join(
      (enter) =>
        enter
          .append("circle")
          .attr("class", "points")
    ...
    //     .attr("d", (d) => areaWithDefinedTrue(d));
    //   d3.select("#linechart g.lines")
    //     .selectAll(".lineElements")
    //     .attr("d", (d) => lineWithDefinedTrue(d));
